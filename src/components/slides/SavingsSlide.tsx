...
export default function SavingsSlide({ slide }: Props) {
 ...
 <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-blue-500/10 rounded-full pointer-events-none -z-10"></div>
 {slide.services?.map((service, index) => (
 <div key={index} className="flex-1 bg-slate-900/40 border border-white/10 hover:border-blue-500/50 rounded-[3rem] p-16 shadow-2xl flex flex-col items-center text-center transform hover:-translate-y-4 transition-all duration-300 relative overflow-hidden group">
